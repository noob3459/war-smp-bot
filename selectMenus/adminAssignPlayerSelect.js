...
module.exports = {
  customId: 'admin_assign_player_select',
  ...
  async execute(interaction, client) {
    ...
    if (count >= team.max_players) {
      return interaction.editReply({
        embeds: [new EmbedBuilder().setTitle('Team Full').setDescription(`**${team.name}** is full.`).setColor(0xe74c3c)],
        components: [],
      });
    }
    ...
    if (alreadyOn) {
      const other = db.prepare('SELECT name FROM teams WHERE id = ?').get(alreadyOn.team_id);
      return interaction.editReply({
        embeds: [new EmbedBuilder().setTitle('Already Assigned').setDescription(`This player is already on **${other?.name ?? 'another team'}**.`).setColor(0xe74c3c)],
        components: [],
      });
    }
