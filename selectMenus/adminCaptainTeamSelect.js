...
module.exports = {
  customId: 'admin_captain_team_select',
  ...
  async execute(interaction, client) {
    ...
    const menu = new StringSelectMenuBuilder()
      .setCustomId(`admin_captain_member_select:${teamId}`)
      .setPlaceholder('Step 2 — Choose the new captain...')
      .addOptions(members.map(m => ({
        label: m.ign ?? m.username ?? `User ${m.user_id}`,
        description: m.user_id === team.captain_discord_id ? '★ Current Captain' : `Discord: <@${m.user_id}>`,
        value: m.user_id,
      })));

    await interaction.update({
      embeds: [new EmbedBuilder()
        .setTitle('👑  Set Captain — Step 2')
        .setDescription(
          `Team: **${team.name}**\n` +
          `Current Captain: ${team.captain_discord_id ? `<@${team.captain_discord_id}>` : '*None*'}\n\n` +
