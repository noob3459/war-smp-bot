...
module.exports = {
  customId: 'admin_remove_player_select',
  ...
  async execute(interaction, client) {
    ...
    const wasCaptain = team.captain_discord_id === userId;
    db.prepare('DELETE FROM team_members WHERE user_id = ?').run(userId);
    if (wasCaptain) db.prepare('UPDATE teams SET captain_discord_id = NULL WHERE id = ?').run(team.id);
    ...
    try {
      ...
      if (wasCaptain && captainRole) await member.roles.remove(captainRole).catch(() => {});
    } catch {}
    ...
    await notifyStaff(client, {
      ...
      description: `**${displayName}** was removed from **${team.name}**.`,
      color: 0xe74c3c,
      fields: [
    ...
    await interaction.editReply({
      embeds: [new EmbedBuilder()
        ...
        .addFields({ name: 'Minecraft', value: app?.ign ?? 'N/A', inline: true })
